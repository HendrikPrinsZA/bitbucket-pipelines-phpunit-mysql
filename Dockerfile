...
RUN \
 ...
 npm install --no-color --production --global gulp-cli webpack mocha grunt clean-css uglify-js &&\
 rm -rf /root/.npm /root/.composer /tmp/* /var/lib/apt/lists/*

RUN \
 curl -o ~/sencha-cmd.run.zip http://cdn.sencha.com/cmd/6.2.2/no-jre/SenchaCmd-6.2.2-linux-amd64.sh.zip && unzip -p ~/sencha-cmd.run.zip > ~/sencha-cmd.sh && chmod +x ~/sencha-cmd.sh && ./sencha-cmd.sh -q && PATH="/root/bin/Sencha/Cmd/6.5.1.240:$PATH"
